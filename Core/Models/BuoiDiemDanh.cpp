...
Status ChiTietDiemDanh::getTrangThai() const { return _trangThai; }
...
const std::string &ChiTietDiemDanh::getGhiChu() const { return _ghiChu; }
...
void ChiTietDiemDanh::setTrangThai(Status trangThai) { _trangThai = trangThai; }
...
BuoiDiemDanh::BuoiDiemDanh(DateTime ngayDiemDanh, CaHoc caDiemDanh, int soTiet)
    : _ngayDiemDanh(std::move(ngayDiemDanh)), _caDiemDanh(caDiemDanh),
      _soTiet(soTiet) {}
...
CaHoc BuoiDiemDanh::getCaDiemDanh() const { return _caDiemDanh; }
...
int BuoiDiemDanh::getSoTiet() const { return _soTiet; }

bool BuoiDiemDanh::isKhoaDiemDanh() const { return _khoaDiemDanh; }
...
void BuoiDiemDanh::capNhatTrangThai(const std::string &maSV, Status trangThai) {
  ...
  _danhSachChiTiet[it->second].setTrangThai(trangThai);
}

void BuoiDiemDanh::capNhatGhiChu(const std::string &maSV,
                                 const std::string &ghiChu) {
  if (_khoaDiemDanh)
    throw std::runtime_error("Buổi điểm danh đã bị khóa!");
...
nlohmann::json BuoiDiemDanh::toJson() const {
  ...
  nlohmann::json j = {
      ...
      {"chiTiet", dump_array<ChiTietDiemDanh>(
                      _danhSachChiTiet, [](const ChiTietDiemDanh &chiTiet) {
...
BuoiDiemDanh BuoiDiemDanh::fromJson(const nlohmann::json &j) {
  ...
  buoi._danhSachChiTiet =
      load_array<ChiTietDiemDanh>(j, "chiTiet", [](const nlohmann::json &item) {
        return ChiTietDiemDanh::fromJson(item);
      });

  for (std::size_t i = 0; i < buoi._danhSachChiTiet.size(); ++i)
    buoi._svIndex[buoi._danhSachChiTiet[i].getMaSV()] = i;
...
BuoiDiemDanh::findChiTiet(const std::string &maSV) const {
  auto it = _svIndex.find(maSV);
  if (it == _svIndex.end())
    return nullptr;
  return &_danhSachChiTiet[it->second];
}

bool BuoiDiemDanh::daDiemDanh(const std::string &maSV) const {
  return _svIndex.count(maSV) > 0;
}
