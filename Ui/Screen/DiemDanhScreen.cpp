...
static void screenDiemDanhBuoi(AppManager &app, const std::string &maLHP,
  ...
  while (!thoat) {
    ...
    for (const auto &maSV : dsMaSV) {
      ...
      Status tt = (itTT != trangThaiMap.end()) ? itTT->second : Status::DEFAULT;

      entries.push_back(statusStr(tt) + "  " + maSV + "  " + ten);
    }
    ...
    auto btnCoMat = Button("Có mặt [1]", [&] {
      ...
      try {
        if (trangThaiMap.count(maSV))
          app.getDDManager().capNhatTrangThai(maLHP, buoiIndex, maSV,
                                              Status::CO_MAT);
        else
          app.getDDManager().diemDanh(maLHP, buoiIndex, maSV, DateTime(),
                                      Status::CO_MAT, "");
    ...
    if (isAdmin) {
      ...
      layout = Container::Vertical(
          {menuSV, Container::Horizontal({btnCoMat, btnVang, btnMuon}),
            ...
            [&] {
              return vbox(
                  {UiHelper::makeHeader("ĐIỂM DANH", headerSub), separator(),
                   ...
                   hbox({
                       text("  Có mặt: ") | dim,
                       text(std::to_string(soCoMat)) | color(Color::Green) |
                           bold,
                       text("  Vắng: ") | dim,
                       text(std::to_string(soVang)) | color(Color::Red) | bold,
                       text("  Muộn: ") | dim,
                       text(std::to_string(soMuon)) | color(Color::Yellow) |
                           bold,
                       text("  Chưa ĐD: ") | dim,
                       text(std::to_string(soChuaDD)) | color(Color::GrayDark) |
                           bold,
                   }) | center,
                   separator(),
                   // Danh sách SV
                   vbox({menuSV->Render() | flex}) | border | flex, separator(),
                   // Nút điểm danh
                   khoa ? text(" Buổi này đã bị khóa - không thể điểm danh "
                               "thêm ") |
                              color(Color::Red) | center
                        : [&]() -> Element {
                     if (isAdmin) {
                       return vbox(
                           {text(" CHẾ ĐỘ: XEM & QUẢN LÝ ") | bold |
                                color(Color::Yellow) | center,
                            separator(),
                            hbox({khoa ? btnMoKhoa->Render()
                                       : text(" [Chưa khóa] ") | dim,
                                  text(" "), btnXoaBuoi->Render(), text(" "),
                                  !khoa ? btnKhoa->Render()
                                        : text(" [Đã khóa] ") | dim,
                   ...
                   UiHelper::makeFooter(
                       isAdmin
                           ...
                           : (khoa ? "[Q] Quay lại"
                                   : "[1] Có mặt  [2] Vắng  [3] Muộn  [A] Tất "
                                     "cả  [K] Khóa  [Q] Quay lai"))});
            }) |
        CatchEvent([&](Event e) {
          if (isAdmin) {
            if (e == Event::Character('m') || e == Event::Character('M')) {
              btnMoKhoa->OnEvent(Event::Return);
              return true;
            }
            if (e == Event::Character('x') || e == Event::Character('X')) {
              btnXoaBuoi->OnEvent(Event::Return);
              return true;
            }
            if (e == Event::Character('k') || e == Event::Character('K')) {
              btnKhoa->OnEvent(Event::Return);
              return true;
            }
          } else if (!khoa) {
            if (e == Event::Character('1')) {
              btnCoMat->OnEvent(Event::Return);
              return true;
            }
            if (e == Event::Character('2')) {
              btnVang->OnEvent(Event::Return);
              return true;
            }
            if (e == Event::Character('3')) {
              btnMuon->OnEvent(Event::Return);
              return true;
            }
            if (e == Event::Character('a') || e == Event::Character('A')) {
              btnTatCaCoMat->OnEvent(Event::Return);
              return true;
            }
            if (e == Event::Character('k') || e == Event::Character('K')) {
              btnKhoa->OnEvent(Event::Return);
