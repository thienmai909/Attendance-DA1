...
void screenGiangVien(AppManager &app) {
  ...
  while (!thoat) {
    ...
    auto thaoTacEntries = std::vector<std::string>{
        "[T] Thêm GV", "[S] Sửa GV", "[X] Xóa GV", "[Q] Quay lại"};
    ...
    auto renderer =
        Renderer(
            ...
            [&] {
              ...
              if (!danhSachGV.empty() &&
                  selected < static_cast<int>(danhSachGV.size())) {
                const auto &giangVien = danhSachGV[selected];
                int soLop =
                    static_cast<int>(app.getLHPManager()
                                         .getLopTheoGV(giangVien.getMaGV())
                                         .size());
                auto lhGV = giangVien.getLienHe();

                chiTiet = vbox(
                    {text(" CHI TIẾT ") | bold | center, separator(),
                     hbox({text(" Mã GV      : ") | dim,
                           text(giangVien.getMaGV()) | bold}),
                     hbox({text(" Họ tên     : ") | dim,
                           text(giangVien.getHoTenGV()) | bold}),
                     hbox({text(" Học vị     : ") | dim,
                           text(giangVien.getHocViStr())}),
                     hbox({text(" Tài khoản  : ") | dim,
              ...
              return vbox(
                  {UiHelper::makeHeader("QUẢN LÝ GIẢNG VIÊN"), separator(),
                   hbox({vbox({text(" THAO TÁC ") | bold | center, separator(),
                         ...
                         vbox({hbox({text(" DANH SÁCH (") | dim,
                                     text(std::to_string(danhSachGV.size())) |
                                         bold,
                                     text(") ") | dim}) |
                                   center,
                               separator(), menuGV->Render() | flex}) |
                             border | flex | size(WIDTH, EQUAL, 60),

                         chiTiet | border | size(WIDTH, EQUAL, 42)}) |
                       flex,
                   separator(),
                   UiHelper::makeFooter(
                       "[↑↓] Chọn  [T]hêm  [S]ửa  [X]óa  [Q]uay lại")});
            }) |
        CatchEvent([&](Event e) {
    ...
    std::string maGVChon =
        (!danhSachGV.empty() && selected < static_cast<int>(danhSachGV.size()))
            ? danhSachGV[selected].getMaGV()
            : "";

    switch (luaChon) {
    case 0:
      formThemGiangVien(app);
      break;
    case 1:
      if (!maGVChon.empty())
        formSuaGiangVien(app, maGVChon);
      break;
    case 2:
      if (!maGVChon.empty()) {
