...
BaoCaoManager::BaoCaoManager(ThongKeManager &tkManager,
                             std::filesystem::path outputDir)
    : _tkManager(tkManager), _outputDir(std::move(outputDir)) {}

void BaoCaoManager::xuatBaoCaoBuoi(const std::string &maLHP,
                                   std::size_t buoiIndex,
                                   DinhDangXuat dinhDang) {
  ensureOutputDir();
  auto path =
      taoTenFile(maLHP, "buoi_" + std::to_string(buoiIndex + 1), dinhDang);
...
void BaoCaoManager::xuatBaoCaoTatCaBuoi(const std::string &maLHP,
                                        DinhDangXuat dinhDang) {
  ensureOutputDir();
...
void BaoCaoManager::xuatBaoCaoSinhVien(const std::string &maLHP,
                                       DinhDangXuat dinhDang) {
  ensureOutputDir();
...
void BaoCaoManager::xuatDanhSachCamThi(const std::string &maLHP,
                                       DinhDangXuat dinhDang) {
  ensureOutputDir();
...
std::filesystem::path BaoCaoManager::taoTenFile(const std::string &maLHP,
...
std::filesystem::path BaoCaoManager::taoTenFile(const std::string &loai,
                                                DinhDangXuat dinhDang) {
  std::string ext = (dinhDang == DinhDangXuat::CSV)    ? ".csv"
                    : (dinhDang == DinhDangXuat::JSON) ? ".json"
                                                       : ".xlsx";
...
std::filesystem::path BaoCaoManager::taoTenFileXLSX(const std::string &maLHP,
                                                    const std::string &loai) {
  return _outputDir / (maLHP + "_" + loai + "_" + taoTimestamp() + ".xlsx");
}

void BaoCaoManager::xuatBuoiCSV(const std::string &maLHP, std::size_t buoiIndex,
...
void BaoCaoManager::xuatTatCaBuoiCSV(const std::string &maLHP,
                                     const std::filesystem::path &filePath) {
  auto dsBuoi = _tkManager.thongKeTatCaBuoi(maLHP);

  utility_csv::CSVData data;
  data.headers = {"Buoi",  "Ngay", "Ca",   "SoTiet",
                  "CoMat", "Vang", "Muon", "TyLeCoMat"};
  for (const auto &buoi : dsBuoi)
    data.rows.push_back(
        {std::to_string(buoi.buoiIndex + 1), buoi.ngay, buoi.ca,
         std::to_string(buoi.soTiet), std::to_string(buoi.soCoMat),
         std::to_string(buoi.soVang), std::to_string(buoi.soMuon),
         std::to_string(static_cast<int>(buoi.tyLeCoMat * 100)) + "%"});
  utility_csv::write_csv(filePath, data);
}

void BaoCaoManager::xuatSinhVienCSV(const std::string &maLHP,
                                    const std::filesystem::path &filePath) {
  auto dsSV = _tkManager.thongKeToanLop(maLHP);

  utility_csv::CSVData data;
  data.headers = {"MaSV",       "HoTen",    "SoTietCoMat", "SoTietVang",
                  "SoTietMuon", "TyLeVang", "BiCamThi"};
  for (const auto &sinhVien : dsSV)
    data.rows.push_back(
        {sinhVien.maSV, sinhVien.tenSV, std::to_string(sinhVien.soTietCoMat),
...
void BaoCaoManager::xuatCamThiCSV(const std::string &maLHP,
                                  const std::filesystem::path &filePath) {
  auto dsSV = _tkManager.dsSVBiCamThi(maLHP);

  utility_csv::CSVData data;
  data.headers = {"MaSV", "HoTen", "SoTietVang", "TyLeVang"};
  for (const auto &sinhVien : dsSV)
    data.rows.push_back(
        {sinhVien.maSV, sinhVien.tenSV, std::to_string(sinhVien.soTietVang),
         std::to_string(static_cast<int>(sinhVien.tyLeVang * 100)) + "%"});
  utility_csv::write_csv(filePath, data);
}

void BaoCaoManager::xuatTongQuanCSV(const std::filesystem::path &filePath) {
  auto dsLop = _tkManager.thongKeTatCaLop();

  utility_csv::CSVData data;
  data.headers = {"MaLHP",  "TenLHP",     "SoSV",      "SoBuoi",
                  "SoTiet", "TyLeVangTB", "SoSVCamThi"};
  for (const auto &lop : dsLop)
    data.rows.push_back(
        {lop.maLHP, lop.tenLHP, std::to_string(lop.soSinhVien),
         std::to_string(lop.soBuoiDaHoc), std::to_string(lop.soTietDaHoc),
         std::to_string(static_cast<int>(lop.tyLeVangTrungBinh * 100)) + "%",
         std::to_string(lop.soSVBiCamThi)});
  utility_csv::write_csv(filePath, data);
}

void BaoCaoManager::xuatBuoiJSON(const std::string &maLHP,
                                 std::size_t buoiIndex,
                                 const std::filesystem::path &filePath) {
  auto tkB = _tkManager.thongKeBuoi(maLHP, buoiIndex);
  auto bcB = _tkManager.baoCaoBuoi(maLHP, buoiIndex);
  ...
  for (const auto &[maSV, status] : bcB)
    j["chiTiet"].push_back(
        {{"maSV", maSV}, {"trangThai", statusToStr(status)}});
  utility_json::write_json(filePath, j);
}

void BaoCaoManager::xuatTatCaBuoiJSON(const std::string &maLHP,
                                      const std::filesystem::path &filePath) {
  auto dsBuoi = _tkManager.thongKeTatCaBuoi(maLHP);
  ...
  for (const auto &buoi : dsBuoi)
    j["buoi"].push_back({{"buoi", buoi.buoiIndex + 1},
                         {"ngay", buoi.ngay},
                         {"ca", buoi.ca},
                         {"soTiet", buoi.soTiet},
...
void BaoCaoManager::xuatSinhVienJSON(const std::string &maLHP,
                                     const std::filesystem::path &filePath) {
  auto dsSV = _tkManager.thongKeToanLop(maLHP);
  ...
  for (const auto &sinhVien : dsSV)
    j["sinhVien"].push_back({{"maSV", sinhVien.maSV},
...
void BaoCaoManager::xuatCamThiJSON(const std::string &maLHP,
                                   const std::filesystem::path &filePath) {
  auto dsSV = _tkManager.dsSVBiCamThi(maLHP);
  ...
  for (const auto &sinhVien : dsSV)
    j["camThi"].push_back({{"maSV", sinhVien.maSV},
                           {"tenSV", sinhVien.tenSV},
                           {"soTietVang", sinhVien.soTietVang},
                           {"tyLeVang", sinhVien.tyLeVang}});
...
void BaoCaoManager::xuatTongQuanJSON(const std::filesystem::path &filePath) {
  auto dsLop = _tkManager.thongKeTatCaLop();
  ...
  for (const auto &lop : dsLop)
    j["tongQuan"].push_back({{"maLHP", lop.maLHP},
                             {"tenLHP", lop.tenLHP},
...
static void writeHeaderRow(lxw_worksheet *ws, lxw_format *fmt,
  ...
  for (int c = 0; c < (int)headers.size(); ++c) {
    worksheet_write_string(ws, 0, (lxw_col_t)c, headers[c].c_str(), fmt);
...
static lxw_row_t writeInfoBlock(lxw_worksheet *ws, lxw_workbook *wb,
                                const LopHocPhan &lhp, const std::string &tenGV,
                                const std::string &timestamp, int nCT,
                                int soSV) {
  lxw_format *fTitle = workbook_add_format(wb);
  format_set_bold(fTitle);
  format_set_font_size(fTitle, 13);
  ...
  format_set_bold(fLabel);
  format_set_bg_color(fLabel, 0xD6E4F0);
  ...
  int pctTienDo =
      lhp.getTongSoTiet() > 0
          ? (int)(100.0 * lhp.getSoTietDaHoc() / lhp.getTongSoTiet())
          : 0;
  std::string tenPhong =
      lhp.getTenPhongHoc().empty() ? "(Chưa có)" : lhp.getTenPhongHoc();

  // Row 0: Title merged
  int cols = 6;
  worksheet_merge_range(
      ws, 0, 0, 0, cols - 1,
      ("BÁO CÁO ĐIỂM DANH - " + lhp.getMaLHP() + " - " + lhp.getTenLHP())
          .c_str(),
      fTitle);
  ...
  std::vector<KV> right = {
      {"Ngưỡng cấm thi:",
       std::to_string(nguongPct) + "% (max " + std::to_string(maxVang) +
           " tiết)",
       false},
      {"Tổng số tiết:", std::to_string(lhp.getTongSoTiet()) + " tiết", false},
      {"Tiến độ học tập:",
       std::to_string(lhp.getSoTietDaHoc()) + "/" +
           std::to_string(lhp.getTongSoTiet()) + " tiết (" +
           std::to_string(pctTienDo) + "%)",
       false},
  ...
  for (int r = 0; r < maxR; ++r) {
    ...
    if (r < (int)left.size()) {
      ...
      worksheet_write_string(ws, row, 1, left[r].v.c_str(),
                             left[r].isWarn ? fWarn : fVal);
    }
    if (r < (int)right.size()) {
      worksheet_write_string(ws, row, 2, right[r].k, fLabel);
      worksheet_write_string(ws, row, 3, right[r].v.c_str(),
                             right[r].isWarn ? fWarn : fVal);
    }
  }
  ...
  format_set_italic(fDim);
  format_set_font_color(fDim, 0x808080);
...
void BaoCaoManager::xuatSinhVienXLSX(const std::string &maLHP,
                                     const std::filesystem::path &filePath) {
  auto dsSV = _tkManager.thongKeToanLop(maLHP);
  ...
  lxw_row_t startRow =
      writeInfoBlock(ws, wb, lhp, "", taoTimestamp(), nCT, (int)dsSV.size());

  lxw_format *fHeader = workbook_add_format(wb);
  format_set_bold(fHeader);
  format_set_bg_color(fHeader, 0x2E75B6);
  ...
  format_set_bg_color(fCT, 0xFF0000);
  format_set_font_color(fCT, LXW_COLOR_WHITE);
  format_set_bold(fCT);
  format_set_border(fCT, LXW_BORDER_THIN);
  lxw_format *fCon = workbook_add_format(wb);
  format_set_bg_color(fCon, 0xE2EFDA);
  format_set_border(fCon, LXW_BORDER_THIN); // xanh la nhat
  lxw_format *fConBad = workbook_add_format(wb);
  format_set_bg_color(fConBad, 0xFFCCCC);
  format_set_border(fConBad, LXW_BORDER_THIN); // do nhat
  ...
  for (int c = 0; c < (int)hdrs.size(); ++c)
    worksheet_write_string(ws, startRow, (lxw_col_t)c, hdrs[c].c_str(),
                           fHeader);
  ...
  for (int i = 0; i < (int)dsSV.size(); ++i) {
    ...
    worksheet_write_string(ws, row, 7, conStr.c_str(),
                           con > 0 ? fCon : fConBad);
  ...
  format_set_bold(fSum);
  format_set_bg_color(fSum, 0xD6E4F0);
  format_set_border(fSum, LXW_BORDER_THIN);
  worksheet_write_string(ws, rowSum, 0, "TỔNG KẾT", fSum);
  worksheet_write_string(ws, rowSum, 1,
                         (std::to_string(dsSV.size()) + " SV").c_str(), fSum);
  worksheet_write_string(ws, rowSum, 3,
                         (std::to_string(tongVang) + " tiết").c_str(), fSum);
  worksheet_write_string(
      ws, rowSum, 8, ("Cấm thi: " + std::to_string(nCT) + " SV").c_str(), fSum);

  workbook_close(wb);
}

void BaoCaoManager::xuatTatCaBuoiXLSX(const std::string &maLHP,
                                      const std::filesystem::path &filePath) {
  auto dsBuoi = _tkManager.thongKeTatCaBuoi(maLHP);
  ...
  writeHeaderRow(
      ws, fHeader,
      {"Buổi", "Ngày", "Ca", "Số Tiết", "Có Mặt", "Vắng", "Muộn", "% Có Mặt"});
...
void BaoCaoManager::xuatCamThiXLSX(const std::string &maLHP,
                                   const std::filesystem::path &filePath) {
  auto dsSV = _tkManager.dsSVBiCamThi(maLHP);
  lxw_workbook *wb = workbook_new(filePath.string().c_str());
  ...
  lxw_format *fTitle = workbook_add_format(wb);
  format_set_bold(fTitle);
  format_set_font_size(fTitle, 14);
  format_set_bg_color(fTitle, 0xC00000); // do dam
  format_set_font_color(fTitle, LXW_COLOR_WHITE);
  format_set_align(fTitle, LXW_ALIGN_CENTER);
  format_set_align(fTitle, LXW_ALIGN_VERTICAL_CENTER);
  worksheet_merge_range(ws, 0, 0, 0, 4,
                        ("DANH SÁCH SINH VIÊN BỊ CẤM THI - " + maLHP).c_str(),
                        fTitle);
  worksheet_set_row(ws, 0, 28, nullptr);

  lxw_format *fHeader = workbook_add_format(wb);
  format_set_bold(fHeader);
  format_set_bg_color(fHeader, 0xC00000);
  format_set_font_color(fHeader, LXW_COLOR_WHITE);
  format_set_border(fHeader, LXW_BORDER_THIN);
  format_set_align(fHeader, LXW_ALIGN_CENTER);
  ...
  const std::vector<std::string> hdrs = {"STT", "Mã SV", "Họ Tên", "Tiết Vắng",
                                         "% Vắng"};
  for (int c = 0; c < 5; ++c)
...
void BaoCaoManager::xuatTongQuanXLSX(const std::filesystem::path &filePath) {
  ...
  lxw_format *fHeader = workbook_add_format(wb);
  format_set_bold(fHeader);
  format_set_bg_color(fHeader, 0x1F4E79);
  format_set_font_color(fHeader, LXW_COLOR_WHITE);
  format_set_border(fHeader, LXW_BORDER_THIN);
  format_set_align(fHeader, LXW_ALIGN_CENTER);
  ...
  writeHeaderRow(ws, fHeader,
                 {"Mã LHP", "Tên LHP", "Số SV", "Số Buổi", "Tiết Đã Học",
                  "% Vắng TB", "SV Cấm Thi"});
...
void BaoCaoManager::xuatToanDienXLSX(const std::string &maLHP,
                                     const std::filesystem::path &filePath) {
  auto dsSV = _tkManager.thongKeToanLop(maLHP);
  auto dsBuoi = _tkManager.thongKeTatCaBuoi(maLHP);
  ...
  // ---- SHEET 1: Tong Quan LHP ----
  {
    ...
    worksheet_merge_range(
        ws, 0, 0, 0, 3,
        ("BÁO CÁO ĐIỂM DANH - " + maLHP + " - " + tkLop.tenLHP).c_str(),
        fTitle);
    ...
    format_set_font_color(fWarnCell, 0xC00000);
    format_set_bold(fWarnCell);
    ...
    std::vector<KV2> right2 = {
        {"Ngưỡng cấm thi:",
         std::to_string(nguongPct) + "% (tối đa " + std::to_string(maxVangLop) +
             " tiết)",
         false},
        {"Tổng số tiết:", std::to_string(lhp.getTongSoTiet()) + " tiết", false},
        {"Tiến độ học tập:",
        ...
        {"Số buổi đã học:",
         std::to_string(tkLop.soBuoiDaHoc) + " buổi / " +
             std::to_string(tkLop.soTietDaHoc) + " tiết",
         false},
    };
    ...
    for (int r = 0; r < maxR2; ++r) {
      ...
      if (r < (int)right2.size()) {
        ...
        worksheet_write_string(ws, row, 3, right2[r].v.c_str(),
                               right2[r].warn ? fWarnCell : mkCell());
      }
    }
    ...
    format_set_bold(fSub);
    format_set_bg_color(fSub, 0x2E75B6);
    format_set_font_color(fSub, LXW_COLOR_WHITE);
    format_set_border(fSub, LXW_BORDER_THIN);
    worksheet_merge_range(ws, rowSep, 0, rowSep, 3,
                          "THỐNG KÊ TÌNH TRẠNG SINH VIÊN", fSub);
    ...
    for (int n = 0; n < (int)nhoms.size(); ++n) {
      ...
      std::string pctStr =
          soSVSheet1 > 0
              ? std::to_string((int)(100.0 * nhoms[n].so / soSVSheet1)) +
                    "% số SV"
              : "-";
    ...
    worksheet_write_string(ws, rowPct, 0,
                           "% Vắng trung bình toàn lớp:", fLabel);
    ...
    format_set_italic(fDim2);
    format_set_font_color(fDim2, 0x808080);
  ...
  // ---- SHEET 2: Diem Danh Buoi ----
  {
    ...
    writeHeaderRow(ws, fH,
                   {"Buổi", "Ngày", "Ca", "Số Tiết", "Có Mặt", "Vắng", "Muộn",
                    "% Có Mặt"});
    worksheet_set_column(ws, 1, 1, 14, nullptr);
    ...
    // Ty le den lop TB = (co mat + muon) / tong luot diem danh
    // Mau = tongCoMat + tongMuon + tongVang (tong luot SV duoc diem danh)
    int tongLuot = tongCoMat + tongMuon + tongVang;
    double tbCoMat = tongLuot > 0
        ? (double)(tongCoMat + tongMuon) / tongLuot
        : 0.0;
  ...
  // ---- SHEET 4: Cam Thi ----
  {
    ...
    format_set_bold(fT2);
    format_set_bg_color(fT2, 0xC00000);
    ...
    worksheet_merge_range(
        ws, 0, 0, 0, 4,
        ("DANH SÁCH SINH VIÊN BỊ CẤM THI - " + maLHP + " - " + tkLop.tenLHP)
            .c_str(),
        fT2);
    worksheet_set_row(ws, 0, 28, nullptr);
