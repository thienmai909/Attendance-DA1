...
class DangKyHocManager {
  ...
  std::unordered_map<std::string, std::vector<std::string>> _indexByLHP;
  void _rebuildIndex();

public:
  explicit DangKyHocManager(
      std::filesystem::path filePath = "data/dangkyhoc.json");
  ...
  getDsMaSVTheoLop(const std::string &maLHP) const;
...
private:
  std::vector<DangKyHoc>::iterator timIterator(const std::string &maSV,
